...
with tabs[1]:
    ...
    if uploaded_file:
        ...
        if "machine_type" not in df.columns:
            st.error("CSV must include a 'machine_type' column (as string)")
        else:
            ...
            # Drop string column and use encoded label
            input_df = df.drop(columns=["machine_type"])
...
with tabs[2]:
    ...
    if 'df' in locals():
        ...
        filter_choice = st.selectbox("Filter", ["All", "Only Risky", "High Risk with RUL < 1000"])

        if filter_choice == "Only Risky":
            st.dataframe(df[df["risk_level"] != "Low Risk"])
        elif filter_choice == "High Risk with RUL < 1000":
            st.dataframe(df[(df["risk_level"] == "High Risk") & (df["rul"] < 1000)])
        else:
            st.dataframe(df)
    else:
        st.info("Please upload a CSV in 'Batch Upload' tab to enable charts.")
