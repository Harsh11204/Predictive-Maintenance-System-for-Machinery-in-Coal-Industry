...
machine_map = {"Conveyor belt": 0, "Crusher": 1, "Loader": 2}
...
with tabs[0]:
    ...
    input_data = pd.DataFrame([{
        ...
        "machine_type_label": machine_map[machine_type]
    }])
    ...
    if st.button("🔍 Predict Failure Risk, Type & RUL"):
        ...
        rul = int(rul_model.predict(input_data)[0])
...
with tabs[1]:
    ...
    if uploaded_file:
        ...
        df['machine_type_label'] = df['machine_type'].map(machine_map)
        df['downtime_percentage'] = df['downtime_minutes'] / df['planned_operating_time'] * 100
        df['risk'] = risk_model.predict(df.drop(columns=['machine_type']))
        df['risk_level'] = df['risk'].map({0: 'Low Risk', 1: 'Medium Risk', 2: 'High Risk'})
        df['rul'] = rul_model.predict(df.drop(columns=['machine_type']))
...
with tabs[2]:
    ...
    if 'df' in locals():
        ...
        st.warning("Please upload a CSV in the 'Batch Upload' tab to enable this section.")
